...
class EntryController extends Controller
{
    ...
    public function update(EntryRequest $request, $id)
    {
        ...
        return DB::transaction(function () use ($entry, $request) {
            ...
            $idsEnviados = collect($data['entry_details'])->pluck('ingreso_id')->filter()->toArray();
            logger('IDs recopilados:', $idsEnviados);



            $idsEnviados = collect($data['entry_details'])->pluck('id')->filter()->toArray();
            Log::info("idsEnviados: ", [
            'idsEnviados' => $idsEnviados]);
            ...
            foreach ($entry->entryDetails as $existingDetail) {
                // Si el detalle sigue presente (con ID), actualizar
                Log::info("id: ", [
...
*/
    public function entryDetailsForReentry()
    {
        $entryDetails = EntryDetail::with([
            ...
            ->whereNull('item_id') // Solo ingresos originales, no reingresos

            ->whereHas('entry', function ($q) {
                $q->where('entity_id', auth()->user()->entity_id)
                    ->where('estado_id', 28); // Solo ingresos ACTIVOS
            })
            ->get();
    ...
    public function storeReturn(Request $request)
    {
        ...
        $entry = DB::transaction(function () use ($validated, $user, $entityId) {
            ...
            foreach ($validated['detalles'] as $row) {
                ...
                // Validar que no se devuelva más de lo solicitado (menos lo ya devuelto)
                $solicitado = (int) $dd->cantidad_solicitada;
                $devueltoAcumulado = (int) EntryDetail::where('origen_discharge_detail_id', $dd->id)->sum('cantidad');
                $saldo = $solicitado - $devueltoAcumulado;
